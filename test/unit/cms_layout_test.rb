...
class Jangle::LayoutTest < ActiveSupport::TestCase
  ...
  def test_validation_of_tag_presence
    ...
    layout = jangle_sites(:default).jangle_layouts.new(
      :label    => 'test',
      :slug     => 'test',
  ...
  def test_creation
    assert_difference 'Jangle::Layout.count' do
      layout = jangle_sites(:default).jangle_layouts.create(
        :label    => 'New Layout',
        :slug     => 'new-layout',
  ...
  def test_options_for_select
    assert_equal ['Default Layout', 'Nested Layout', '. . Child Layout'],
      Jangle::Layout.options_for_select(jangle_sites(:default)).collect{|t| t.first}
    assert_equal ['Default Layout', 'Nested Layout'],
      Jangle::Layout.options_for_select(jangle_sites(:default), jangle_layouts(:child)).collect{|t| t.first}
    assert_equal ['Default Layout'],
      Jangle::Layout.options_for_select(jangle_sites(:default), jangle_layouts(:nested)).collect{|t| t.first}
  end
  ...
  def test_load_from_file
    assert !Jangle::Layout.load_from_file(jangle_sites(:default), 'default')
    
    Jangle.configuration.seed_data_path = File.expand_path('../cms_seeds', File.dirname(__FILE__))
    
    assert !Jangle::Layout.load_from_file(jangle_sites(:default), 'bogus')
    
    assert layout = Jangle::Layout.load_from_file(jangle_sites(:default), 'default')
    assert_equal 'Default Layout', layout.label
    assert_equal '<html>{{cms:page:content}}</html>', layout.content
    
    assert layout = Jangle::Layout.load_from_file(jangle_sites(:default), 'nested')
  ...
  def test_load_from_file_broken
    ...
    assert_exception_raised RuntimeError, error_message do
      Jangle::Layout.load_from_file(jangle_sites(:default), 'broken')
    end
  end
  
  def test_load_for_slug
    assert layout = Jangle::Layout.load_for_slug!(jangle_sites(:default), 'default')
  ...
  def test_load_for_slug_exceptions
    assert_exception_raised ActiveRecord::RecordNotFound, 'Jangle::Layout with slug: not_found cannot be found' do
      Jangle::Layout.load_for_slug!(jangle_sites(:default), 'not_found')
    end
    assert !Jangle::Layout.load_for_slug(jangle_sites(:default), 'not_found')
  ...
  def test_update_forces_page_content_reload
    layout_1 = jangle_layouts(:nested)
    layout_2 = jangle_layouts(:child)
    page_1 = jangle_sites(:default).jangle_pages.create!(
      :label          => 'page_1',
      :slug           => 'page-1',
      :parent_id      => jangle_pages(:default).id,
      :jangle_layout_id  => layout_1.id,
      :is_published   => '1',
    ...
    page_2 = jangle_sites(:default).jangle_pages.create!(
      :label          => 'page_2',
      :slug           => 'page-2',
      :parent_id      => jangle_pages(:default).id,
      :jangle_layout_id  => layout_2.id,
      :is_published   => '1',
      :jangle_blocks_attributes => [
        { :label    => 'header',
          :content  => 'header_content' },
